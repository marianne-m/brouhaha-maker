...
## How to get predictions from a pre-trained VAD / snr / reverb estimator ?

To get Voice Activitation Detection (VAD), signal-to-noise ratio (snr) and intellibility (C_50) estimations, you can run :

```bash
python CPC_torch/cpc/eval/inference_vad.py --pathDB $PATH_AUDIO_FILES --pathPretrained $PATH_PRETRAINED --pathOut $PATH_OUT --file_extension .wav \
  --ignore_cache --hiddenGar 512 --hiddenEncoder 512
```

This will create one .pt file per audio file in the $PATH_OUT folder.
Each .pt file will contain :
1) A vector of speech classes, 1 score per frame : 1 if there's speech, 0 if not.
2) An estimated snr, 1 score per audio file.
3) An estimated C_50, 1 score per audio file.

Estimated snr and C_50 are calculated on 1.28 seconds long windows. 
The score obtained for each window is then averaged to obtain a score at the audio file level.
If you want all scores, you can run the command with the ```--no_sentence_level``` parameter.

For devfair users : a pretrained model can be found under `/private/home/marvinlvn/CPC_audio_jzay/AlodieVAD/model/CPC_libribig_600_noise_smooth_reverb_reverse_FT_librispeech/checkpoint_24.pt`
...
```bash
python cpc/eval/common_voices_eval.py train $PATH_COMMON_VOICES/es/clips_16k $PATH_COMMON_VOICES/es/validated_phones_reduced.txt $CHECKPOINT_TO_TEST --pathTrain $PATH_COMMON_VOICES/es/trainSeqs_5.0_uniform_new_version.txt --pathVal $PATH_COMMON_VOICES/es/valSeqs_5.0_uniform_new_version.txt --freeze -o $OUTPUT_DIR
```
