...
def extract_subset(
    ...
    if gender_file is not None:
        gender_index = sd.get_genders_from_file(speakers, gender_file)
        gender_sizes = sd.get_gender_sizes(speaker_sizes, gender_index)
        min_gender = np.argmin(gender_sizes)
        target_time_genders = [0, 0]
        target_time_genders[min_gender] = min(target_time / 2, gender_sizes[min_gender])
        target_time_genders[1 - min_gender] = (
            target_time - target_time_genders[min_gender]
        )
        ...
        for _g in range(2):
            ...
            speaker_gender = min(n_speakers // 2, np.sum(sizes_speakers_of_gender > 0))
    ...
    for loc_speaker_size, loc_target_time, loc_target_n_speakers in speaker_groups:
        ...
        if random_sampling:
            speaker_list = samplers.get_top_n_speakers(
                loc_speaker_size, loc_target_n_speakers
            )
            loc_seq = sd.filter_by_speaker(sequence_data, speaker_list)
            loc_train_sequences, loc_remainers = samplers.random_sampling(
                loc_seq, loc_target_time
            )
        else:
            loc_speaker_selection = samplers.estimate_balanced_speakers(
                loc_speaker_size,
                loc_target_n_speakers,
                loc_target_time,
                args.min_time_speaker,
            )
            loc_train_sequences, loc_remainers = samplers.filter_by_speaker_time_target(
                sequence_data, loc_speaker_selection
            )
...
def parse_args(argv):
    ...
    parser = argparse.ArgumentParser(
        description="A tools which build a dataset " "split from its statistics"
    )

    parser.add_argument(
        "path_stats_dir",
        help="Path to the directory containing the statistics " "on the dataset",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a file detailling the gender of the " "speakers",
    )
    parser.add_argument(
        "--target_time",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="If > 0, remove all sequences with an energy" "below the given threshold",
    )
    parser.add_argument(
        "--path_filter",
        type=str,
    ...
    parser.add_argument(
        "--random_sampling",
        action="store_true",
        help="If activated, do not try to get a balanced " "speaker distribution",
    )
    parser.add_argument(
        "--ignore",
        type=str,
        default=None,
        nargs="*",
        help="Exclude the given sequences from the " "sampling",
    )
...
def main(args):
    ...
    if args.top_perplexity is not None:
        sequence_data = samplers.get_top_by_attr(
            sequence_data, "perplexity", args.top_perplexity * 3600
        )
    if args.worst_perplexity is not None:
        sequence_data = samplers.get_top_by_attr(
            sequence_data, "perplexity", args.worst_perplexity * 3600, reverse=True
        )
    ...
    if args.threshold_energy is not None:
        sequence_data = [x for x in sequence_data if x.energy > args.threshold_energy]

    print(f"Saving the selection at {args.output}")

    if args.target_time_test > 0:
        print("Test set")
        selected_sequences_test, sequence_data = extract_subset(
            sequence_data,
            speakers,
            args.target_time_test,
            args.n_speakers,
            random_sampling=args.random_sampling,
            gender_file=args.gender_file,
        )
        path_test = Path(args.output).parent / f"{Path(args.output).stem}_test.txt"
        sd.save_seq_list(selected_sequences_test, path_test)

    if args.target_time_dev > 0:
        print("Dev set")
        selected_sequences_dev, sequence_data = extract_subset(
            sequence_data,
            speakers,
            args.target_time_dev,
            args.n_speakers,
            random_sampling=args.random_sampling,
            gender_file=args.gender_file,
        )
        path_dev = Path(args.output).parent / f"{Path(args.output).stem}_dev.txt"
        sd.save_seq_list(selected_sequences_dev, path_dev)

    if args.target_time > 0:
        print("Training set")
        selected_sequences_train, sequence_data = extract_subset(
            sequence_data,
            speakers,
            args.target_time,
            args.n_speakers,
        ...
        path_train = Path(args.output).parent / f"{Path(args.output).stem}_train.txt"
        sd.save_seq_list(selected_sequences_train, path_train)

    if args.target_time_val > 0:
        print("Validation set")
        selected_sequences_val, _ = extract_subset(
            sequence_data,
            speakers,
            args.target_time_val,
            args.n_speakers,
            random_sampling=args.random_sampling,
            gender_file=args.gender_file,
        )
        path_val = Path(args.output).parent / f"{Path(args.output).stem}_val.txt"
