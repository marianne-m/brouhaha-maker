...
class CPCBertModel(CPCModel):
    def __init__(self, encoder, AR, nMaskSentence=2, blockSize=12):
    ...
    def getMask(self, batchData):
        ...
        maskLabel = torch.randint(
            0, seqSize // self.blockSize, (self.nMaskSentence * batchSize, 1)
        )
        ...
        baseY = torch.arange(0, batchSize, dtype=torch.long).view(-1, 1) * seqSize
        baseY = (
            baseY.expand(batchSize, self.nMaskSentence * self.blockSize)
            .contiguous()
            .view(-1)
        )
        maskLabel = maskLabel + baseY
        outLabels = torch.zeros(batchSize * seqSize, dtype=torch.uint8)
