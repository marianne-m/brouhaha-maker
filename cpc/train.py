...
def getCriterion(args, downsampling, nSpeakers, nPhones, inference_mode=False):
    dimFeatures = args.hiddenGar if not args.onEncoder else args.hiddenEncoder
    if hasattr(args, 'supervised') and not args.supervised:
    ...
    if not type(cpcCriterion).__name__ == 'PhoneCriterion' and inference_mode:
        raise ValueError("inference_mode = True is only supported for PhoneCriterion.")
    return cpcCriterion


def loadCriterion(args, pathCheckpoint, downsampling, nSpeakers, nPhones, inference_mode=False):
    _, _, locArgs = fl.getCheckpointData(os.path.dirname(pathCheckpoint))
    locArgs.hiddenEncoder = args.hiddenEncoder
    locArgs.hiddenGar = args.hiddenGar
    criterion = getCriterion(locArgs, downsampling, nSpeakers, nPhones, inference_mode=inference_mode)
...
def loadCriterionSNR(args, pathCheckpoint, inference_mode=False):
    try:
        _, _, locArgs = fl.getCheckpointData(os.path.dirname(pathCheckpoint))
        locArgs.hiddenEncoder = args.hiddenEncoder
        locArgs.hiddenGar = args.hiddenGar
        dimFeatures = locArgs.hiddenGar if not locArgs.onEncoder else locArgs.hiddenEncoder
        criterion_snr = cr.SNRCriterion(int(locArgs.sizeWindow/160), dimFeatures, locArgs.onEncoder, inference_mode=inference_mode)

        state_dict = torch.load(pathCheckpoint, 'cpu')

        criterion_snr.load_state_dict(state_dict["snrCriterion"])
        return criterion_snr
    except:
        return None


def loadCriterionReverb(args, pathCheckpoint, inference_mode=False):
    try:
        _, _, locArgs = fl.getCheckpointData(os.path.dirname(pathCheckpoint))
        locArgs.hiddenEncoder = args.hiddenEncoder
        locArgs.hiddenGar = args.hiddenGar
        dimFeatures = locArgs.hiddenGar if not locArgs.onEncoder else locArgs.hiddenEncoder
        criterion_reverb = cr.ReverbCriterion(int(locArgs.sizeWindow/160), dimFeatures, locArgs.onEncoder, inference_mode=inference_mode)

        state_dict = torch.load(pathCheckpoint, 'cpu')

        criterion_reverb.load_state_dict(state_dict["reverbCriterion"])
        return criterion_reverb
    except:
        return None
...
def adversarialTrainStep(dataLoader, cpcModel,
    ...
    for step, full_data in enumerate(dataLoader):
        ...
        sequence, label, _ = [x.cuda(non_blocking=True) for x in full_data]
...
def trainStep(dataLoader,
    ...
    iter = 0
    for step, full_data in enumerate(dataLoader):
        sequence, label, _, _ = [x.cuda(non_blocking=True) for x in full_data]
        past, future = sequence[:, 0, ...], sequence[:, 1, ...]
...
def run(trainDataset,
    ...
    for epoch in range(startEpoch, nEpoch):
        ...
        if pathCheckpoint is not None: #\
            ...
            fl.save_checkpoint(modelStateDict, criterionStateDict, None,
                               None, optimizer.state_dict(), bestStateDict,
...
def main(argv):
    ...
    if args.load is not None and args.loadCriterion:
        cpcCriterion = loadCriterion(args, args.load[0], cpcModel.gEncoder.DOWNSAMPLING,
