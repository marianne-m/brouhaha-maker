...
from cpc.model import MFCCEncoder, CPCModel, NoAr
...
def ABX(
    ...
    if "within" in modes:
        ...
        scores["within"] = (phone_confusion.sum() / (divisor_speaker > 0).sum()).item()
    ...
    if "across" in modes:
        ...
        ABXIterator = ABXDataset.get_iterator(
            "across", max_size_group, max_x_across=max_x_across
        )
        ...
        phone_confusion = reduce_sparse_data(
            group_confusion.sum(dim=0).sum(dim=2), divisor_speaker
        )
        scores["across"] = (phone_confusion.sum() / (divisor_speaker > 0).sum()).item()
...
def update_base_parser(parser):
    ...
    parser.add_argument(
        "--feature_size", type=float, default=0.01, help="Size (in s) of one feature"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Max size of a group while computing the" "ABX score",
    )
    parser.add_argument(
        "--max_x_across",
        type=int,
    ...
    parser.add_argument(
        "--out", type=str, default=None, help="Path where the results should be saved"
    )
...
def parse_args(argv):
    ...
    parser_checkpoint.add_argument(
        "path_checkpoint", type=str, help="Path to the model's checkpoint"
    )
    parser_checkpoint.add_argument(
        "path_item_file",
        type=str,
    ...
    parser_checkpoint.add_argument("path_dataset", type=str, help="Path to the dataset")
    parser_checkpoint.add_argument(
        "--seq_norm",
        action="store_true",
    ...
    parser_checkpoint.add_argument(
        ...
        type=str,
        default=".wav",
        help="Extension of ecah audio file in the " "dataset.",
    )
    parser_checkpoint.add_argument(
        "--get_encoded",
        action="store_true",
    ...
    parser_db.add_argument(
        "path_features", type=str, help="Path to pre-computed torch features (.pt)"
    )
    parser_db.add_argument(
        "--file_extension",
        type=str,
    ...
    parser_mfcc = subparsers.add_parser("mfcc")
    parser_mfcc.add_argument(
        "path_item_file",
        type=str,
        help="Path to the ABX .item file containing " "the triplets labels",
    )
    parser_mfcc.add_argument("path_dataset", type=str, help="Path to the dataset")
    parser_mfcc.add_argument(
        "--file_extension",
        type=str,
        default=".wav",
    ...
    update_base_parser(parser_mfcc)
...
def main(argv):
    ...
    if args.load == "from_checkpoint":
        ...
        def feature_function(x):
            return torch.load(x, "cpu")

    elif args.load == "mfcc":
        model = CPCModel(MFCCEncoder(256), NoAr())
        feature_maker = FeatureModule(model, True).cuda().eval()
    ...
    seq_list, _ = findAllSeqs(
        args.path_dataset, extension=args.file_extension, speaker_level=-1
    )
    seq_list = [
        (str(Path(x).stem), str(Path(args.path_dataset) / x)) for (_, x) in seq_list
    ]
    ...
    out_dir = Path(args.path_checkpoint).parent if args.out is None else Path(args.out)
